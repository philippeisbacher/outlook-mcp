...
async function handleListFolders(args) {
  ...
  try {
    ...
    const folders = await getAllFoldersHierarchy(accessToken, includeItemCounts, mailbox);
    ...
    if (includeChildren) {
      return {
        content: [{
          type: "text",
          text: formatFolderHierarchy(folders, includeItemCounts, mailboxInfo)
        }]
      };
    } else {
      ...
      return {
        content: [{
          type: "text",
          text: formatFolderList(folders, includeItemCounts, mailboxInfo)
        }]
      };
    }
  } catch (error) {
 ...
 */
async function getAllFoldersHierarchy(accessToken, includeItemCounts, mailbox = null) {
 ...
 * @param {string} mailboxInfo - Additional mailbox info string
 * @returns {string} - Formatted list
 */
function formatFolderList(folders, includeItemCounts, mailboxInfo = '') {
  if (!folders || folders.length === 0) {
    return `No folders found${mailboxInfo}.`;
  }
  ...
  return `Found ${folders.length} folders${mailboxInfo}:\n\n${folderLines.join('\n')}`;
}
 ...
 */
function formatFolderHierarchy(folders, includeItemCounts, mailboxInfo = '') {
  if (!folders || folders.length === 0) {
    return `No folders found${mailboxInfo}.`;
  }
  ...
  return `Folder Hierarchy${mailboxInfo}:\n\n${formattedHierarchy}`;
}
