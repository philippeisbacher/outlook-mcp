...
describe('handleListEmails', () => {
  ...
  describe('successful email retrieval', () => {
    ...
    test('should list emails from specified folder', async () => {
      ...
      expect(resolveFolderPath).toHaveBeenCalledWith(mockAccessToken, customFolder, null);
      expect(callGraphAPIPaginated).toHaveBeenCalledWith(
        mockAccessToken,
        'GET',
  ...
  describe('shared mailbox support', () => {
    test('should pass mailbox parameter to resolveFolderPath', async () => {
      const sharedMailbox = 'shared@company.com';
      ensureAuthenticated.mockResolvedValue(mockAccessToken);
      resolveFolderPath.mockResolvedValue(`users/${sharedMailbox}/mailFolders/inbox/messages`);
      callGraphAPIPaginated.mockResolvedValue({ value: mockEmails });

      const result = await handleListEmails({ mailbox: sharedMailbox });

      expect(resolveFolderPath).toHaveBeenCalledWith(mockAccessToken, 'inbox', sharedMailbox);
